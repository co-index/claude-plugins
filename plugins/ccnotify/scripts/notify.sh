...
# Claude Code Notification/Stop hook: posts a macOS notification through the
# ccnotify helper (https://github.com/co-index/ccnotify). Clicking the banner
# jumps back to the app that was running Claude Code. Falls back to
# terminal-notifier, then to a non-clickable osascript notification.
...
if notifier:
    ...
    # Fallback: native AppleScript notification (not clickable). Install the
    # helper for clickable banners: brew install co-index/tap/ccnotify
    script = """
on run argv
